...
let emit_instruction =
  ...
  | UnaryRegisterInstruction {operation; destination_register; source_register } ->
      sprintf "%4s %s, %s"
        (string_of_operation operation)
        (emit_operand destination_register)
        (emit_operand source_register)
  | BinaryRegisterInstruction { operation; destination_register; source_register_1; source_register_2 } ->
      sprintf "%4s %s, %s, %s"
        (string_of_operation operation)
        (emit_operand destination_register)
...
let rec compile program =
  ...
  [section `Text; global start; tl_label start] 
...
let emit_program program = compile program |> List.map emit |> CCList.to_string (Fun.id) ~sep:"\n"
